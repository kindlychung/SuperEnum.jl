...
macro se(T, syms...)
    ...
    for s in syms
        if s isa Symbol
            push!(vals, (s,i, string(s)))
        elseif s isa String
            push!(vals, (Symbol(Base.replace(s, r"[^a-zA-Z]" => "")),i, s))
        elseif (s isa Expr) && s.head == :call && s.args[1] == :(=>) && s.args[2] isa Symbol && s.args[3] isa String
            # is this robust? what if there is a linenumbernode?
            # what if in x => y, y is an expression?
            push!(vals, (s.args[2], i, s.args[3]))
        end
        if length(vals) == 1
            lo = hi = i
        else
